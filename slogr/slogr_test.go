...
func ExampleNewSlogLogger() {
	...
	logger := slog.New(slogr.NewSlog(funcrLogger))
	logger.Info("hello world")
	...
	logger = slog.New(slogr.NewSlog(funcrLogger.V(int(-slog.LevelDebug))))
...
func TestSlogHandler(t *testing.T) {
	...
	handler := slogr.NewSlog(funcrLogger)
...
func TestDiscard(t *testing.T) {
	logger := slog.New(slogr.NewSlog(logr.Discard()))
	logger.WithGroup("foo").With("x", 1).Info("hello")
}

func TestConversion(t *testing.T) {
	d := logr.Discard()
	d2 := slogr.NewLogr(slogr.NewSlog(d))
	expectEqual(t, d, d2)

	e := logr.Logger{}
	e2 := slogr.NewLogr(slogr.NewSlog(e))
	expectEqual(t, e, e2)

	f := funcr.New(func(prefix, args string) {}, funcr.Options{})
	f2 := slogr.NewLogr(slogr.NewSlog(f))
	expectEqual(t, f, f2)

	text := slog.NewTextHandler(io.Discard, nil)
	text2 := slogr.NewSlog(slogr.NewLogr(text))
	expectEqual(t, text, text2)
	text3 := slogr.NewSlog(slogr.NewLogr(text).V(1))
